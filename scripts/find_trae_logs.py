#!/usr/bin/env python3
"""Find TRAE workspace databases from the last N days."""
import json
import os
import platform
import sys
...
def trae_user_dir():
    ...
    if platform.system() == 'Darwin':
        return home / 'Library' / 'Application Support' / 'Trae' / 'User'
    if platform.system() == 'Windows':
        appdata = os.environ.get('APPDATA')
        return Path(appdata) / 'Trae' / 'User' if appdata else home / 'AppData' / 'Roaming' / 'Trae' / 'User'
    return home / '.config' / 'Trae' / 'User'


def find_fallback_dirs():
    home = Path.home()
    names = ['vibecheck-logs', 'trae-logs']
...
def find_workspace_dbs(root_dir, days=14):
    ...
    elif (root_dir / 'Trae' / 'User' / 'workspaceStorage').exists():
        workspace_root = root_dir / 'Trae' / 'User' / 'workspaceStorage'
        global_db = root_dir / 'Trae' / 'User' / 'globalStorage' / 'state.vscdb'
    else:
        workspace_root = root_dir / 'workspaceStorage'
        global_db = root_dir / 'globalStorage' / 'state.vscdb'
...
def main():
    ...
    if not sessions:
        print(json.dumps({
            ...
            'tool': 'trae',
            'export_script': 'scripts/export_logs.py trae',
            'dest': '~/vibecheck-logs',
            'sessions': [],
            'total_sessions': 0,
        }, indent=2))
