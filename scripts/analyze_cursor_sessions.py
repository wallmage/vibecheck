#!/usr/bin/env python3
"""Analyze Cursor chat/composer SQLite data for cost and waste patterns."""
import json
import math
import os
import re
...
def normalize_model(model):
    ...
    for pattern, key in MODEL_PATTERNS:
        if pattern in model:
            return key
    # Cursor commonly routes through Claude-family models when no explicit model is stored.
    return 'sonnet'
...
def build_analysis_confidence(results):
    if not results:
        return {
            'label': 'estimated',
            'score': 0.4,
            'reason': 'No parsed Cursor conversations.',
        }
    token_rich = sum(1 for r in results if r.get('total_cache_read', 0) or r.get('total_cache_create', 0))
    share = token_rich / len(results)
    models = {r.get('model', 'sonnet') for r in results}
    if share >= 0.7 and all(get_billing_mode(model) == 'full_billing' for model in models):
        return {
            'label': 'partial',
            'score': 0.8,
            'reason': 'Most Cursor sessions include token counters or cache signals, and the detected models are in the full-billing frontier set.',
        }
    return {
        'label': 'estimated',
        'score': 0.64,
        'reason': 'Cursor analysis often reconstructs token counts from SQLite chat content, so totals remain partly estimated.',
    }
...
def read_item_table_value(conn, key):
    tables = list_tables(conn)
    if 'ItemTable' not in tables:
        return None
    for column in ('key', '[key]'):
        try:
            row = conn.execute(f"SELECT value FROM ItemTable WHERE {column} = ?", (key,)).fetchone()
            if row:
                return row[0]
...
def read_cursor_disk_kv(conn, pattern=None):
    tables = list_tables(conn)
    if 'cursorDiskKV' not in tables:
        return []
    try:
        if pattern:
            return conn.execute("SELECT key, value FROM cursorDiskKV WHERE key LIKE ?", (pattern,)).fetchall()
        return conn.execute("SELECT key, value FROM cursorDiskKV").fetchall()
    except sqlite3.DatabaseError:
        return []
...
def load_workspace_composers(workspace_db):
    conn = open_db(workspace_db)
    try:
        raw = read_item_table_value(conn, 'composer.composerData')
        if raw:
            data = parse_json_maybe(raw)
            if isinstance(data, dict) and isinstance(data.get('allComposers'), list):
                return data.get('allComposers', [])

        raw = read_item_table_value(conn, 'workbench.panel.aichat.view.aichat.chatdata')
        data = parse_json_maybe(raw)
        if isinstance(data, dict):
            chats = data.get('allComposers') or data.get('composers') or data.get('chats') or []
...
def load_global_composer_data(global_db):
    if not global_db or not Path(global_db).exists():
        return {}, {}
    conn = open_db(global_db)
    try:
        composer_rows = read_cursor_disk_kv(conn, 'composerData:%')
        bubble_rows = read_cursor_disk_kv(conn, 'bubbleId:%')
        composers = {}
        ...
        for row in bubble_rows:
            ...
            data = parse_json_maybe(value)
            if isinstance(data, dict):
                bubbles[composer_id][bubble_id] = data
...
def detect_role(bubble):
    role_keys = ['role', 'sender', 'author', 'messageRole']
    for path, value in recursive_walk(bubble):
        key = path.rsplit('.', 1)[-1]
        if key in role_keys and isinstance(value, str):
            lower = value.lower()
            if 'assistant' in lower or 'ai' == lower:
                return 'assistant'
            if 'user' in lower or 'human' in lower:
                return 'user'
    texts = ' '.join(collect_strings(bubble)[:10]).lower()
    if '"assistant"' in texts:
        return 'assistant'
    if '"user"' in texts:
        return 'user'
    return 'assistant'
...
def extract_text_and_metrics(bubble):
    strings = collect_strings(bubble)
    combined = '\n'.join(strings)
    model = None
    created_at = None
    command_texts = []
    ...
    for path, value in recursive_walk(bubble):
        key = path.rsplit('.', 1)[-1]
        if key in ('model', 'modelName', 'model_name', 'modelId') and isinstance(value, str):
            model = value
        elif key in ('createdAt', 'timestamp', 'time') and isinstance(value, str) and created_at is None:
            created_at = value
        elif key in ('command', 'cmd', 'terminalCommand') and isinstance(value, str):
...
def build_conversations(workspace_entry):
    composers = load_workspace_composers(workspace_entry['path'])
    global_composers, bubbles_by_composer = load_global_composer_data(workspace_entry.get('global_db'))
    conversations = []

    for composer in composers:
        composer_id = composer.get('composerId') or composer.get('id')
        if not composer_id:
            continue
        global_data = global_composers.get(composer_id, {})
        headers = global_data.get('fullConversationHeadersOnly') or composer.get('fullConversationHeadersOnly') or []
        bubble_ids = [h.get('bubbleId') for h in headers if isinstance(h, dict) and h.get('bubbleId')]
        if not bubble_ids and composer_id in bubbles_by_composer:
            bubble_ids = list(bubbles_by_composer[composer_id].keys())

        bubbles = []
        for bubble_id in bubble_ids:
            bubble = bubbles_by_composer.get(composer_id, {}).get(bubble_id)
            if bubble:
        ...
        conversation = {
            ...
            'name': composer.get('name') or global_data.get('name') or 'Untitled',
            'created_at': composer.get('createdAt') or global_data.get('createdAt') or workspace_entry['timestamp'],
            'bubbles': bubbles,
...
def detect_platform(conversation_metrics):
...
def analyze_conversation(conversation):
    ...
    for bubble in conversation['bubbles']:
        role = detect_role(bubble)
        info = extract_text_and_metrics(bubble)
        info['role'] = role
        metrics.append(info)
        role_counts[role] += 1
    ...
    model = normalize_model(next((m['model'] for m in metrics if m['model']), None))
    assistant_messages = [m for m in metrics if m['role'] == 'assistant']
    user_messages = [m for m in metrics if m['role'] == 'user']
    ...
    for metric in assistant_messages:
        ...
        is_idle = (
            len(text.strip()) > 0
            and len(text_tokens.to_bytes(2, 'little', signed=False)) >= 0  # no-op to keep branch simple
            and len(text) < 280
            and any(phrase in lower_text for phrase in ("i'll", "i will", "going to", "next i'll", "let me"))
            and not commands
    ...
    if sum(1 for cmd in all_commands if cmd.strip().startswith(('cat ', 'sed ', 'head ', 'tail ', 'rg ', 'find ', 'ls '))) >= 5:
        codebase_wandering = max(0, sum(1 for cmd in all_commands if cmd.strip().startswith(('cat ', 'sed ', 'head ', 'tail ', 'rg ', 'find ', 'ls '))) - 3)
    ...
    return {
        ...
        'waste': {
            'idle_narration': {
                ...
                'description': 'Short assistant messages that appear to narrate the next action instead of doing it',
            },
            'chainable_bash': {
                'turns': chainable,
        ...
        'date': (conversation['created_at'] or conversation['workspace_entry']['date'])[:10],
    }
...
def main():
    if len(sys.argv) < 2:
        print("Usage: analyze_cursor_sessions.py <sessions.json>")
        sys.exit(1)
    ...
    for entry in workspace_entries:
        conversations = build_conversations(entry)
    ...
    if not results:
        print(json.dumps({'error': 'No valid Cursor conversations parsed'}))
        sys.exit(1)
